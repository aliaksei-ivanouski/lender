...
| ADR-014 | Attendee registration: Fortify-authenticated, confirmed on creation | Accepted | 2026-06-18 | Schema: `event_registrations(id, user_id FK, event_id FK, status, reminder_sent_3d, reminder_sent_24h, created_at, updated_at)` with unique constraint on (user_id, event_id). Auth gated: POST/DELETE `/events/{event}/registrations` requires Fortify login; guests redirected. Confirmation email sent once on wasRecentlyCreated. |
